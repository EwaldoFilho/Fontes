...
function zoomProdutoModal(obj) {
    ...
    var fl = $("#codFilial").val();
    var filial = fl.substring(0,2);
    var CC = $("#codCentroCusto").val();
    var filters = "FILIAL," + filial + ",CC," + CC;    
    var searchby = "FILTRO";
    
    tdizoom.open(
        "dsProdutosCusto",
        "CODIGO,Código,DESCRICAO,Descrição,UNIDADE,Unidade",
        "CODIGO,DESCRICAO,TIPOCC",
...
function zoomCusto(obj) {	
    ...
    if (estado == "0" || estado == "4") {
    tdizoom.open(
        "dsCentroCustoPCO", //Nome do Dataset
...
function setSelectedZoomItem(selectedItem){
    ...
    if(name == "produto"){
	    ...
	    $("#tipoCC").val(selectedItem.TIPOCC);
