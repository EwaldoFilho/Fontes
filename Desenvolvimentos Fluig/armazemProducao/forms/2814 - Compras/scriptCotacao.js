   ...
   // var numeroProcesso = "3727";
    ...
    function getDadosCotacoes(){
        try {
            ...
            var constraintsCotacoes = new Array(constraintNumProc, constraintFilial);      
        
            var datasetCotacoes = DatasetFactory.getDataset("armazem_buscar_cotacao", null, constraintsCotacoes, null);
       ...
       var taxaFrete = "0.0";
    	...
    	for(var countProduto=0; countProduto<produtosFormatos.length; countProduto++){
          ...
          fornecedores = fornecedores.map(function(x,y){
            ...
            x.precoTotal = x.precoTotal.replace(",", ".");
        	...
        	for(var countFornecedor=0; countFornecedor<fornecedores.length; countFornecedor++){
               ...
               totalSolicitacaoCompra = (parseFloat(totalSolicitacaoCompra) + parseFloat(fornecedores[countFornecedor].precoTotal));
        		}
        	}    		
    	}
    ...
    function formatarProdutos(cotacoesValues){
        ...
        for (var countProduto = 0; countProduto < cotacoesValues.length; countProduto++) {
            produto = JSON.parse('{"codProduto":"'+cotacoesValues[countProduto].C8_PRODUTO+'","produto":"'+cotacoesValues[countProduto].B1_DESC+'", "quantidade":"'+parseInt(cotacoesValues[countProduto].C8_QUANT)+'", "codArmazem":"'+cotacoesValues[countProduto].CP_LOCAL+'", "lojaForn":"'+cotacoesValues[countProduto].C8_LOJA+'", "filial":"'+cotacoesValues[countProduto].C8_FILIAL+'", "codNumCotacao":"'+cotacoesValues[countProduto].C8_NUM+'", "codNumSc":"'+cotacoesValues[countProduto].C8_ITEMSC+'", "totalDespesas":"'+cotacoesValues[countProduto].C8_DESPESA+'", "totalDescontos":"'+cotacoesValues[countProduto].C8_VLDESC+'", "totalSeguro":"'+cotacoesValues[countProduto].C8_SEGURO+'", "totalFrete":"'+cotacoesValues[countProduto].C8_TOTFRE+'", "fornecedor":[], "fornecedores":[], "ganhador":false}');
    	...
    	for (var countFornecedor = 0; countFornecedor <  cotacoesValues.length; countFornecedor++) {        
            for (var countProduto2 = 0; countProduto2 < produtosFormatado.length; countProduto2++) {
               ...
               if(produtosFormatado[countProduto2].fornecedor.length === 0){
                   fornecedor = JSON.parse("{"+'"codItemCotacao":"'+cotacoesValues[countFornecedor].C8_ITEM+'", "fornecedor":"'+cotacoesValues[countFornecedor].C8_FORNECE+'", "precoUnitario":"'+precoUnitario.toFixed(2).toString().replace(".", ",")+'", "precoTotalFornecedor":"'+precoTotalFornecedor.toFixed(2).toString().replace(".", ",")+'","precoTotal":"'+parseFloat(cotacoesValues[countFornecedor].C8_TOTAL).toFixed(2)+'","precoUnitarioFormatado":"'+parseFloat(cotacoesValues[countFornecedor].C8_PRECO).toFixed(2)+'", "totalDespesas":"'+totalDespesa.toFixed(2).toString().replace(".", ",")+'", "totalDescontos":"'+totalDesconto.toFixed(2).toString().replace(".", ",")+'", "totalSeguro":"'+totalSeguro.toFixed(2).toString().replace(".", ",")+'","taxaFrete":"'+taxaFrete.toFixed(2).toString().replace(".", ",")+'", "ganhador":false, "prazoEntrega":"'+cotacoesValues[countFornecedor].E4_DESCRI+'", "codProduto":"'+cotacoesValues[countFornecedor].C8_PRODUTO+'", "nomeFornecedor":"'+cotacoesValues[countFornecedor].A2_NOME+'", "condPagamento":"'+cotacoesValues[countFornecedor].C8_COND+'", "lojaForn":"'+cotacoesValues[countFornecedor].C8_LOJA+'"}');
                     
                 }else{
                   fornecedor = JSON.parse("{"+'"codItemCotacao":"'+cotacoesValues[countFornecedor].C8_ITEM+'", "fornecedor":"'+cotacoesValues[countFornecedor].C8_FORNECE+'","precoUnitarioFormatado":"'+parseFloat(cotacoesValues[countFornecedor].C8_PRECO).toFixed(2)+'", "precoUnitario":"'+precoUnitario.toFixed(2).toString().replace(".", ",")+'", "precoTotalFornecedor":"'+precoTotalFornecedor.toFixed(2).toString().replace(".", ",")+'","precoTotal":"'+parseFloat(cotacoesValues[countFornecedor].C8_TOTAL).toFixed(2)+'", "totalDespesas":"'+totalDespesa.toFixed(2).toString().replace(".", ",")+'", "totalDescontos":"'+totalDesconto.toFixed(2).toString().replace(".", ",")+'", "totalSeguro":"'+totalSeguro.toFixed(2).toString().replace(".", ",")+'","taxaFrete":"'+taxaFrete.toFixed(2).toString().replace(".", ",")+'", "ganhador":false, "prazoEntrega":"'+cotacoesValues[countFornecedor].E4_DESCRI+'", "codProduto":"'+cotacoesValues[countFornecedor].C8_PRODUTO+'", "nomeFornecedor":"'+cotacoesValues[countFornecedor].A2_NOME+'", "condPagamento":"'+cotacoesValues[countFornecedor].C8_COND+'", "lojaForn":"'+cotacoesValues[countFornecedor].C8_LOJA+'"}');
                 }
    ...
    function escolherProposta(idProposta){
        ...
        for (var x = 0; x < fornecedoresModal.length; x++) {            
           ...
           cotacoesAtualizadasJson.push(JSON.parse("{"+'"codItemCotacao":"'+row.codItemCotacao+'", "codProduto":"'+row.codProduto+'","fornecedor":"'+row.fornecedor+'","ganhador":'+row.ganhador+',"nomeFornecedor":"'+row.nomeFornecedor+'","prazoEntrega":"'+row.prazoEntrega+'", "precoTotal":"'+row.precoTotal+'", "precoUnitarioFormatado":"'+row.precoUnitarioFormatado+'", "precoUnitario":"'+row.precoUnitario+'", "precoTotalFormatado":"'+row.precoTotalFormatado+'", "totalDespesas":"'+row.totalDespesas+'", "totalDescontos":"'+row.totalDescontos+'", "totalSeguro":"'+row.totalSeguro+'","taxaFrete":"'+row.taxaFrete+'", "condPagamento":"'+row.condPagamento+'", "lojaForn":"'+row.lojaForn+'"}'));
           cotacoesFormatJson.push("{"+'"codItemCotacao":"'+row.codItemCotacao+'", "codProduto":"'+row.codProduto+'","fornecedor":"'+row.fornecedor+'","ganhador":'+row.ganhador+',"nomeFornecedor":"'+row.nomeFornecedor+'","prazoEntrega":"'+row.prazoEntrega+'", "precoTotal":"'+row.precoTotal+'", "precoUnitario":"'+row.precoUnitario+'", "precoUnitarioFormatado":"'+row.precoUnitarioFormatado+'", "precoTotalFormatado":"'+row.precoTotalFormatado+'", "totalDespesas":"'+row.totalDespesas+'", "totalDescontos":"'+row.totalDescontos+'", "totalSeguro":"'+row.totalSeguro+'","taxaFrete":"'+row.taxaFrete+'", "condPagamento":"'+row.condPagamento+'", "lojaForn":"'+row.lojaForn+'"}');
        }
                          ...
                          '<div class="form-group col-sm-12">'+                                    
                               ...
                               '<div class="input-group col-sm-12">'+
                                '  <span>R$'+fornecedores[count].precoUnitario+'</span>'+                                                                                                                  
                                   ...
                                   '<span>R$'+fornecedores[count].precoTotalFornecedor+'</span>'+   
                                '</div>'+
