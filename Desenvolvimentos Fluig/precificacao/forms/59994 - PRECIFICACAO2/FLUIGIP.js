...
function FLUIGIP(){
	 ...
	 * @example FLUIGIP.setWriteFieldByClass('.amu-control');
     ...
     * @description Remove o alerta de erro de validação de cada input, quando o mesmo é alterado.  Por padrão, o método é chamado automaticamente para todos os inputs que possuam a classe amu-control ou que sejam do tipo rádio e checkbox.
	 * @memberof FLUIGIP
	...
	this.removeAlertValidation = function(){
		$(document).on('change', '.amu-control',function(){
	...
	/**
     * Função setReadOnly
     * @description Atribui a propriedade readOnly a todos os campos que possuem a classe ou o id informado. Utilize o nome da classe com ponto (ex .amu-control) ou o id com # (ex #userName).
	 * @memberof FLUIGIP
     * @param {string} selector Seletor do(s) campo(s) a ser(em) alterado(s)
	 * @example FLUIGIP.setReadOnly(".amu-control"); FLUIGIP.setReadOnly("#userName"); 
	 * @example FLUIGIP, readOnly, classe
	 * @example 
	 * @author
     */
						...
						if ( value.trim() == "" || value =='undefined' || value == null || FLUIGIP.USEFUL.validFloatValue(value) ){
							...
							if (($(this).closest('table').length > 0 && $(this).attr('id').indexOf('___') == -1 )){
								...
								if ($(this).attr("type") == "zoom"){
									...
									$(this).parent("td").find("span[role='combobox']").addClass("has-error-table");
								} else if ($(this).closest('.input-group').length > 0) {
									if ($(this).attr("id").indexOf("___") != -1){
										$(this).addClass("has-error-table");
									} else {
										$(this).closest(".input-group").addClass("has-error-table");
									}
								} else {
									if ($(this).attr("id").indexOf("___") != -1){
										$(this).addClass("has-error-table");
									} else {
										//$(this).parent("td").addClass("has-error-table");
										$(this).addClass("has-error-table");
									}
								}
							}
...
$(document).on("ready", function() {
	...
	FLUIGIP.setReadOnly(".amu-control");
...
$('<style type="text/css"> .has-error-table {border-bottom: 1px solid #a94442 !important;} </style>').appendTo("head");
...
var tdizoom = (function () {
	...
	return {
		...
		open: function (dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby) {
			...
			html += "<input type='text' class='amu-control' id='search' value='" + $("#" + type).val() + "' placeholder='Digite o texto e utilize o <Enter> para buscar'>";
			html += "</div>";
			...
			html += " <div>";
			html += "		<span class='amu-control-static pull-left'>";
...
var tdizoomCheck = (function(){
	...
	return {
		open: function(dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby) {
			...
			html += "<div class='input-group'>" ;
			html += "<span class='input-group-addon'><span class='fluigicon fluigicon-search'></span></span>" ;
			html += "<input type='text' class='amu-control' id='search' value='" + ($("#"+type).length > 0 ? $("#"+type).val() : "") + "' placeholder='Digite o texto e utilize o <Enter> para buscar'>";
			html += "</div>" ;
			if (typeof miniZoom != "undefined") {
				html += "<div class='' id='loading-zoom' style='overflow-y: auto; height: 120px;'>";
			} else {
