	...
	this.writeForState = function(){
		...
		$("input,select,textarea").each(function(){
			...
			if ($(this).attr("type") == "zoom"){
				...
				state   	= $(this).parent("div").data("fluig-state-valid");
			} else if($(this).attr("type") == "radio"){
				required 	= $(this).parent("label").parent("div").data("fluig-required");
				state   	= $(this).parent("label").parent("div").data("fluig-state-valid");
			} else if($(this).attr("type") == "checkbox"){
				required 	= $(this).closest(".input-group").data("fluig-required");
				state   	= $(this).closest(".input-group").data("fluig-state-valid");
			} else {
				required 	=  $(this).data("fluig-required");
				state   	=  $(this).data("fluig-state-valid");
			}

			//se as variaveis possuirem valores setados
			if ( (typeof required != 'undefined') && (typeof state != 'undefined') ){

				
				state = (typeof state == "number" ? state.toString()  : state);
				state = state.split(",");

				//Se a variavel required for true? e (o WKNumState  ou * estiverem presentes no array)
				if ( eval(required) && (state.indexOf(getWKNumState()) != -1  || state.indexOf("*") != -1) ) {
