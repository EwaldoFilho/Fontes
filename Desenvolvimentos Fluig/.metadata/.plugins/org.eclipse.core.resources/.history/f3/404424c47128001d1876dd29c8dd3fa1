function myFunctions(){
	...
	Swal.fire('Atenção: '+  'Verificar os templates disponíveis na Intranet ' + 'para a montagem do documento.')
...
function adicionarRateioCusto(CTT_CUSTO,CTT_DESC01) {
    ...
    $("#codCentroCustoBen___" + id).val(CTT_CUSTO);
    $("#descricaoBen___" + id).val(CTT_DESC01);
    $("#valor___" + id).mask('000.000.000.000.000,00', {reverse: true});
    qtd++
    $("#baseRateio").val(qtd);
    $("#codCentroCustoBen___" + id).attr('readonly', true);
    $("#descricaoBen___" + id).attr('readonly', true);
...
function fnCustomDeleteItReqMat(oElement){
    ...
    FLUIGC.message.confirm({
        ...
        if (result){            
            ...
            $("input[id^='descricaoBen___']").each(function(index, value){

                /*Identifica "SEQUENCIA"(Tabela Itens solicitação) inexistente e elimina registro*/
                if ( typeof $("#descricaoBen___" + $(this).val()).val() === "undefined" ) {
