...
function habilitaBtn () {
    ...
    if(form.getValue("centroCusto") != null)
    {
        if(!document.getElementById('bottom').disabled) document.getElementById('bottom').setAttribute('readOnly','');              
    }

    else if(form.getValue("centroCusto") == null)
    {
        if(document.getElementById('bottom').disabled) document.getElementById('bottom').setAttribute('readOnly','');
    }
}
...
function FLUIGIP(){
	 ...
	 * 
	 * @description Verifica se cada campo é obrigatório (fluig-required ==
	 *              true) e válido (fluig-state-valid possui o valor de
	 *              getWKNumState() ou '*' no array). Caso seja, retira o
	 *              atributo readOnly do mesmo.
	 * @memberof FLUIGIP
	...
	this.writeForState = function(){
		...
		$("input,select,textarea").each(function(){
			...
			// verifica o tipo do campo e recebe os valores especificados para
			// fluig-required e fluig-state-valid
			...
			if ( (typeof required != 'undefined') && (typeof states != 'undefined') ){
				...
				// Se a variavel required for true? e (o WKNumState ou *
				// estiverem presentes no array)
	 ...
	 * 
	 * @description Atribui a propriedade readOnly para cada campo que possuir o
	 *              valor de getWKNumState() ou '*' na propriedade
	 *              fluig-state-readonly.
	 * @memberof FLUIGIP
	 ...
	 * 
	 * @description Oculta todos os elementos que possuam no atributo
	 *              data-fluig-state-hide o valor de getWKNumState() ou '*'.
	 *              Oculta todos os elementos que possuam no atribulo
	 *              data-fluig-conditional-hide uma expressão booleana que
	 *              retorne true.
	 * @memberof FLUIGIP
	 ...
	 * 
	 * @description Retira o atributo readOnly de todos os campos que possuem a
	 *              classe informada.
	 * @memberof FLUIGIP
	 * @param {string}
	 *            classeName Classe dos campos que serão alterados
	 ...
	 * 
	 * @description Remove o alerta de erro de validação de cada input, quando o
	 *              mesmo é alterado. Por padrão, o método é chamado
	 *              automaticamente para todos os inputs que possuam a classe
	 *              form-control ou que sejam do tipo rádio e checkbox.
	 * @memberof FLUIGIP
	 ...
	 * 
	 * @description Atribui as classes 'control-label' e 'required' às labels de
	 *              campos obrigatórios e válidos. Entende-se por obrigatório o
	 *              campo que possuir fluig-required == true e por válido aquele
	 *              em que o fluig-state-valid tem em seu array o valor de
	 *              getWKNumState() ou '*'.
	 * @memberof FLUIGIP
	 ...
	 * 
	 * @description Retira a propriedade readOnly de todos os campos que possuam
	 *              no atributo fluig-write o valor de getWKNumState() ou '*'.
	 * @memberof FLUIGIP
	 ...
	 * Função setReadOnly
	 * 
	 * @description Atribui a propriedade readOnly a todos os campos que possuem
	 *              a classe ou o id informado. Utilize o nome da classe com
	 *              ponto (ex .form-control) ou o id com # (ex #userName).
	 * @memberof FLUIGIP
	 * @param {string}
	 *            selector Seletor do(s) campo(s) a ser(em) alterado(s)
	 * @example FLUIGIP.setReadOnly(".form-control");
	 *          FLUIGIP.setReadOnly("#userName");
	 * @example FLUIGIP, readOnly, classe
	 * @example
	 * @author
	 */
	 ...
	 * 
	 * @description Altera a propriedade "readOnly" do objeto, de acordo com os
	 *              parâmetros passados. Além do objeto a ser atualizado, o
	 *              método também recebe o parâmetro que informa se o objeto
	 *              deve ter a propriedade readOnly (true) ou não (false).
	 * @memberof FLUIGIP
	 * @param {obj}
	 *            obj Objeto a ser alterado
	 * @param {boolean}
	 *            change True ou False
	 ...
	 * 
	 * @description Verifica se a variavel informada possui um tipo válido, ou
	 *              seja, se seu tipo é diferente de nulo, indefinido ou vazio.
	 * @deprecated isValid
	 * @memberof FLUIGIP
	 * @param {any}
	 *            conditional Variável a ser verificada
	 * @returns {boolean} True | False
	 ...
	 * Função updCalendar
	 * 
	 * @description Atualiza os campos do tipo calendario. Esse método é chamado
	 *              toda vez que um campo que possua a classe '.calendar' recebe
	 *              o foco. Caso o campo possua o atributo readOnly, o método
	 *              previne seu conteúdo de ser alterado. Caso não possua, o
	 *              método apenas remove o erro de validação, se houver. Por
	 *              padrão, o método é chamado automaticamente para todos os
	 *              inputs que possuam a classe '.calendar'.
	 * @memberof FLUIGIP
	 ...
	 * Função setSwitcher
	 * 
	 * @description Transforma um ou mais campos (checkbox ou radio button) em
	 *              switcher.
	 * @memberof FLUIGIP
	 * @param {string}
	 *            selector seletor do(s) campo(s)
	...
	this.TABLE = {
		 ...
		 * Função getTableSize
		 * 
		 * @description Retorna o total de linhas da tabela, ignorando as linhas
		 *              que fazem parte do cabeçalho.
		 * @memberof TABLE
		 * @param {string}
		 *            tableId Id da tabela
		 * @param {int}
		 *            tamCab Tamanho do cabeçalho, ou seja, linhas a serem
		 *            ignoradas na contagem
		 * @returns {int} Total de linhas que a tabela possui, sem contabilizar
		 *          o cabeçalho
		 ...
		 * Função clearTable
		 * 
		 * @description Apaga todas as linhas da tabela e mostra uma mensagem ao
		 *              finalizar
		 * @memberof TABLE
		 * @param {string}
		 *            tableName tableName da tabela
		 * @param {string}
		 *            msg Mensagem
		 * @example FLUIGIP.TABLE.clearTable("minhaTabela", "Finalizado com
		 *          sucesso!");
		 ...
		 * 
		 * @description Remove a linha especificada da sua tabela. Antes de
		 *              remover a linha, esse método chama a função
		 *              beforeRemoveLineTable_PE(table, id) caso exista no seu
		 *              código javascript. Após a remoção da mesma, o método
		 *              chama a função removeLineTable_PE(table, id).
		 * @memberof TABLE
		 * @param {*}
		 *            oElement Elemento de referência da linha a ser acionado,
		 *            como por exemplo um ícone, ou a referencia da própria
		 *            linha. Ex: document.getElementById("nomeCampo___1").
		 ...
		 * Função addTable
		 * 
		 * @description Adiciona um filho à tabela pai x filho. Após adicionar o
		 *              filho à tabela pai x filho , o método chama a função
		 *              addTable_PE(table, id)
		 * @deprecated addTableChild
		 * @memberof TABLE
		 * @param {string}
		 *            table tableName da tabela
	...
	this.USEFUL = {
		 ...
		 * @memberof USEFUL
		 * @param {*}
		 *            variable Variável a ser verificada
		 * @returns {boolean} Retorna True ou False
		 ...
		 * 
		 * @description Atribui o valor ao campo e muda seu status para
		 *              readOnly, não permitindo edições.
		 * @memberof USEFUL
		 * @param {string}
		 *            fieldName Name do campo
		 * @param {*}
		 *            value Valor a ser atribuido ao campo
		 * @example FLUIGIP.USEFUL.setValueDisableField("telefone", "12
		 *          1234-5678");
		 ...
		 * Função mValor
		 * 
		 * @description Não permite o uso de caracteres não numéricos e adiciona .
		 *              a cada 3 zeros e , para os 2 decimais (ex. 1.000,00).
		 *              Utilize quando quiser obrigar o usuário a informar
		 *              apenas números em um campo monetário.
		 * @memberof USEFUL
		 * @param {string}
		 *            valor Valor a ser padronizado
		 ...
		 * 
		 * @description Não permite o uso de caracteres não numéricos e adiciona
		 *              '.' a cada 3 zeros. Utilize quando quiser obrigar o
		 *              usuário a informar apenas números no campo.
		 * @memberof USEFUL
		 * @param {string}
		 *            valor String a ser padronizada
		 ...
		 * Função mNumerico
		 * 
		 * @description Não permite o uso de caracteres não numéricos. Utilize
		 *              quando quiser obrigar o usuário a informar apenas
		 *              números no campo.
		 * @memberof USEFUL
		 * @param {string}
		 *            valor String a ser padronizada
		 ...
		 * Função mChar
		 * 
		 * @description Aplica padrão de caracteres na string
		 *              (^a-z,A-z,à-ú,À-Ú). Utilize quando quiser obrigar o
		 *              usuário a informar apenas letras no campo.
		 * @memberof USEFUL
		 * @param {string}
		 *            valor String a ser padronizada
		 * @returns {string} String com o novo padrão
		 ...
		 * Função mAlfanum
		 * 
		 * @description Aplica padrão alfanumérico na string (^a-z,A-z,0-9).
		 *              Utilize quando quiser obrigar o usuário a informar
		 ...
		 * @param {string}
		 *            locale Região da data a ser formatada. Default "pt-BR".
		 *            (opcional)
		 ...
		 * 
		 * @description Converte a variável do tipo float em string, limitando
		 *              as casas decimais de acordo com o valor informado.
		 * @memberof USEFUL
		 * @param {float}
		 *            float Variável a ser convertida
		 * @param {int}
		 *            decimal Número de casas decimais a serem preservadas
		 ...
		 * @param {string}
		 *            fromValue Valor a ser substituído
		 * @param {string}
		 *            toValue Valor a substituir
		 * @returns {string} String modificada
		 * @example novaString = FLUIGIP.USEFUL.replaceAll(stringOriginal,
		 *          fromValue, toValue);
		 ...
		 * Função clearFields
		 * 
		 * @description Ao limpar o campo de referência, limpa todos os campos
		 *              que possuam a classe ou o id informado
		 * @memberof USEFUL
		 * @param {string}
		 *            fieldId id do campo de referência
		 * @param {string}
		 *            selector classe ou id dos campos a serem limpos
		 * @example FLUIGIP.USEFUL.clearFields("#cep",".endereco");
		 *          FLUIGIP.USEFUL.clearFields("#cidade","#UF");
		 ...
		 * Função setDateRange
		 * 
		 * @description Habilita um calendário com um espaço de datas
		 *              pré-definido para seleção, ou seja, o usuário só poderá
		 *              escolher uma data que esteja entre a data mínima e a
		 *              data máxima definida. Esse método recebe 3 parâmetros: a
		 *              data de referência, a quantidade de dias anteriores a
		 *              referência e a quantidade de dias posteriores à mesma.
		 * @memberof USEFUL
		 * @param {string}
		 *            calendarId id do campo de referência
		 * @param {string}
		 *            initialDate id do campo de referência
		 * @param {string}
		 *            daysBefore classe ou id dos campos a serem limpos
		 * @param {string}
		 *            daysAfter classe ou id dos campos a serem limpos
		 * @example FLUIGIP.USEFUL.setDateRange("#meu-calendario","05/03/2020",
		 *          2, 10);
		 * @example USEFUL, calendário, range, datas, data inicial, data final,
		 *          seleção
		 * @example
		 * @author Andressa Oliveira
		 */
	...
	this.VALID = {
		 ...
		 * Função validateForm
		 * 
		 * @description Verifica se cada campo obrigatório foi preenchido, caso
		 *              não tenha sido, retorna um vetor (false, "mensagem de
		 *              erro"). Por padrão, o método é chamado automaticamente
		 *              pelo método beforeSendValidate.
		 * @memberof VALID
		 ...
		 * 
		 * @description Verifica se o valor passado já existe na tabela. Caso
		 *              exista, retorna true, caso contrário, retorna false.
		 * @deprecated isDuplicatedOnGrid
		 * @memberof VALID
		 * @param {string}
		 *            idTable Id da tabela
		 * @param {string}
		 *            idField Id do campo
		 * @param {string}
		 ...
		 * @example eDuplicado =
		 *          FLUIGIP.VALID.validDuplicateValueGrid("tabelaClientes",
		 *          "nomeCliente", "Maria"));
		 ...
		 * @returns {boolean} True ou false
		 * @example eDuplicado =
		 *          FLUIGIP.VALID.isDuplicatedOnGrid("tabelaClientes",
		 *          "nomeCliente", "Maria"));
		 * @example VALID, validação, verificação, tabela, grid
		 * @example
		 * @author
	...
	this.ZOOM = {
		 ...
		 * 
		 * @description Altera a propriedade "readOnly" ao campo zoom, de acordo
		 *              com os parâmetros passados: true ou false.
		 * @memberof ZOOM
		 * @param {string}
		 *            instance Id do campo zoom
		 * @param {boolean}
		 ...
		 * @memberof ZOOM
		 * @param {string}
		 *            instance Id do campo tipo zoom
 ...
 * 
 * @description Valida o formulário antes do envio (chamada automaticamente pelo
 *              Fluig ao enviar um form). Antes de validar o formulário, esse
 *              método chama a função beforeValidateForm_PE(numState, nextState)
 *              e antes de efetuar o envio do form chama a função
 *              beforeSendValidate_PE(numState,nextState).
 * @memberof FLUIGIP
 * @param {int}
 *            numState Atividade atual
 * @param {int}
 *            nextState Próxima atividade
...
/*
 * jQuery Mask Plugin v1.14.13 github.com/igorescobar/jQuery-Mask-Plugin
...
var tdizoom = (function () {
	...
	return {
		 ...
		 * Função open
		 * 
		 * @description Aplica a personalização criada pela TOTVSIP ao campo do
		 *              tipo Zoom. Utilize o tdizoom.open para zoom comum e
		 *              tdizoomCheck.open para zoom com opção de multipla
		 *              escolha.
		 * @memberof TDIZOOM
		 * @param {*}
		 *            dataset Dataset a ser consultado
		 * @param {*}
		 *            fields Campos a serem mostrados
		 * @param {*}
		 *            resultfields
		 * @param {*}
		 *            title Título
		 ...
		 * @example tdizoom.open("nomeDataset",
		 *          "campoDataset1,nomeColuna1,campoDataset2,nomeColuna2",
		 *          "campoDataset1,campoDataset3", "Titulo",
		 *          "filtro,valorfiltro", "nome_do_campo");
			...
			var zoommodal = FLUIGC.modal({
				...
				if (err) {
					...
					var readydataset = function (dataset) {
						...
						/*
						 * if(dataset.length == 1){ var row =
						 * globaldataset[$(".table-zoom tbody
						 * .active").data("dataset")]; row["type"] = type;
						 * zoommodal.remove(); setSelectedZoomItem(row);
						 * 
						 * if(isHeaderHide){
		 ...
		 * Função trigger
		 * 
		 * @description Realiza o gatilho do campo zoom personalizado baseado
		 *              nos parametros informados. (chama as funções padrões
		 ...
		 * @param {*}
		 *            type Tipo
		 * @example tdizoom.trigger("nomeDataset", "campoDatasetPesquisar",
		 *          "valorPesquisar", "filtro,valorfiltro", "nome_do_campo");
		 * @example TDIZOOM, ZOOM
		 * @example
		 * @author Alisson Hausmann
		 */
		...
		open: function(dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby) {
			...
			var zoommodal = FLUIGC.modal({
			    ...
			    if(err) {
					...
					/*
					 * var searchtable = function (text) { var table =
					 * $('.table-zoom > tbody');
					 * table.find('tr').each(function(index, row) { var allCells =
					 * $(row).find('td'); if(allCells.length > 0) { var found =
					 * false; allCells.each(function(index, td) { var regExp =
					 * new RegExp(text, 'i'); if(regExp.test($(td).text())) {
					 * found = true; return false; } }); if(found == true){
					 * $(row).show(); $(row).find("td
					 * input").removeClass("hide-line"); }else{ $(row).hide();
