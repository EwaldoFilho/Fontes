...
(function () {
    var bDebug = false; EXIF.Tags = {
        ...
        if (oElement.addEventListener) oElement.addEventListener(strEvent, fncHandler, false); else if (oElement.attachEvent) oElement.attachEvent("on" +
            strEvent, fncHandler)
    } function imageHasData(oImg) { return !!oImg.exifdata } function getImageData(oImg, fncCallback) { BinaryAjax(oImg.src, function (oHTTP) { var oEXIF = findEXIFinJPEG(oHTTP.binaryResponse); oImg.exifdata = oEXIF || {}; if (fncCallback) fncCallback() }) } function findEXIFinJPEG(oFile) {
        var aMarkers = []; if (oFile.getByteAt(0) != 255 || oFile.getByteAt(1) != 216) return false; var iOffset = 2; var iLength = oFile.getLength(); while (iOffset < iLength) {
            if (oFile.getByteAt(iOffset) != 255) {
                if (bDebug) console.log("Not a valid marker at offset " +
                    iOffset + ", found: " + oFile.getByteAt(iOffset)); return false
    ...
    } function readTags(oFile, iTIFFStart,
                    ...
                    case 1: case 7: if (iNumValues == 1) return oFile.getByteAt(iEntryOffset + 8, bBigEnd); else { var iValOffset = iNumValues > 4 ? iValueOffset : iEntryOffset + 8; var aVals = []; for (var n = 0; n < iNumValues; n++)aVals[n] = oFile.getByteAt(iValOffset + n); return aVals } break; case 2: var iStringOffset = iNumValues > 4 ? iValueOffset : iEntryOffset + 8; return oFile.getStringAt(iStringOffset, iNumValues -
                        1); break; case 3: if (iNumValues == 1) return oFile.getShortAt(iEntryOffset + 8, bBigEnd); else { var iValOffset = iNumValues > 2 ? iValueOffset : iEntryOffset + 8; var aVals = []; for (var n = 0; n < iNumValues; n++)aVals[n] = oFile.getShortAt(iValOffset + 2 * n, bBigEnd); return aVals } break; case 4: if (iNumValues == 1) return oFile.getLongAt(iEntryOffset + 8, bBigEnd); else { var aVals = []; for (var n = 0; n < iNumValues; n++)aVals[n] = oFile.getLongAt(iValueOffset + 4 * n, bBigEnd); return aVals } break; case 5: if (iNumValues == 1) return oFile.getLongAt(iValueOffset,
                            bBigEnd) / oFile.getLongAt(iValueOffset + 4, bBigEnd); else { var aVals = []; for (var n = 0; n < iNumValues; n++)aVals[n] = oFile.getLongAt(iValueOffset + 8 * n, bBigEnd) / oFile.getLongAt(iValueOffset + 4 + 8 * n, bBigEnd); return aVals } break; case 9: if (iNumValues == 1) return oFile.getSLongAt(iEntryOffset + 8, bBigEnd); else { var aVals = []; for (var n = 0; n < iNumValues; n++)aVals[n] = oFile.getSLongAt(iValueOffset + 4 * n, bBigEnd); return aVals } break; case 10: if (iNumValues == 1) return oFile.getSLongAt(iValueOffset, bBigEnd) / oFile.getSLongAt(iValueOffset +
                                4, bBigEnd); else { var aVals = []; for (var n = 0; n < iNumValues; n++)aVals[n] = oFile.getSLongAt(iValueOffset + 8 * n, bBigEnd) / oFile.getSLongAt(iValueOffset + 4 + 8 * n, bBigEnd); return aVals } break
                }
        } function readEXIFData(oFile, iStart, iLength) {
            if (oFile.getStringAt(iStart, 4) != "Exif") { if (bDebug) console.log("Not valid EXIF data! " + oFile.getStringAt(iStart, 4)); return false } var bBigEnd; var iTIFFOffset = iStart + 6; if (oFile.getShortAt(iTIFFOffset) == 18761) bBigEnd = false; else if (oFile.getShortAt(iTIFFOffset) == 19789) bBigEnd = true; else {
                if (bDebug) console.log("Not valid TIFF data! (no 0x4949 or 0x4D4D)");
                return false
            } if (oFile.getShortAt(iTIFFOffset + 2, bBigEnd) != 42) { if (bDebug) console.log("Not valid TIFF data! (no 0x002A)"); return false } if (oFile.getLongAt(iTIFFOffset + 4, bBigEnd) != 8) { if (bDebug) console.log("Not valid TIFF data! (First offset not 8)", oFile.getShortAt(iTIFFOffset + 4, bBigEnd)); return false } var oTags = readTags(oFile, iTIFFOffset, iTIFFOffset + 8, EXIF.TiffTags, bBigEnd); if (oTags.ExifIFDPointer) {
                var oEXIFTags = readTags(oFile, iTIFFOffset, iTIFFOffset + oTags.ExifIFDPointer, EXIF.Tags, bBigEnd); for (var strTag in oEXIFTags) {
                    switch (strTag) {
                        case "LightSource": case "Flash": case "MeteringMode": case "ExposureProgram": case "SensingMethod": case "SceneCaptureType": case "SceneType": case "CustomRendered": case "WhiteBalance": case "GainControl": case "Contrast": case "Saturation": case "Sharpness": case "SubjectDistanceRange": case "FileSource": oEXIFTags[strTag] =
                            EXIF.StringValues[strTag][oEXIFTags[strTag]]; break; case "ExifVersion": case "FlashpixVersion": oEXIFTags[strTag] = String.fromCharCode(oEXIFTags[strTag][0], oEXIFTags[strTag][1], oEXIFTags[strTag][2], oEXIFTags[strTag][3]); break; case "ComponentsConfiguration": oEXIFTags[strTag] = EXIF.StringValues.Components[oEXIFTags[strTag][0]] + EXIF.StringValues.Components[oEXIFTags[strTag][1]] + EXIF.StringValues.Components[oEXIFTags[strTag][2]] + EXIF.StringValues.Components[oEXIFTags[strTag][3]]; break
                    }oTags[strTag] = oEXIFTags[strTag]
                }
            } if (oTags.GPSInfoIFDPointer) {
                var oGPSTags =
                    readTags(oFile, iTIFFOffset, iTIFFOffset + oTags.GPSInfoIFDPointer, EXIF.GPSTags, bBigEnd); for (var strTag in oGPSTags) { switch (strTag) { case "GPSVersionID": oGPSTags[strTag] = oGPSTags[strTag][0] + "." + oGPSTags[strTag][1] + "." + oGPSTags[strTag][2] + "." + oGPSTags[strTag][3]; break }oTags[strTag] = oGPSTags[strTag] }
            } return oTags
        } EXIF.getData = function (oImg, fncCallback) { if (!oImg.complete) return false; if (!imageHasData(oImg)) getImageData(oImg, fncCallback); else if (fncCallback) fncCallback(); return true }; EXIF.getTag = function (oImg,
            strTag) { if (!imageHasData(oImg)) return; return oImg.exifdata[strTag] }; EXIF.pretty = function (oImg) { if (!imageHasData(oImg)) return ""; var oData = oImg.exifdata; var strPretty = ""; for (var a in oData) if (oData.hasOwnProperty(a)) if (typeof oData[a] == "object") strPretty += a + " : [" + oData[a].length + " values]\r\n"; else strPretty += a + " : " + oData[a] + "\r\n"; return strPretty }; EXIF.readFromBinaryFile = function (oFile) { return findEXIFinJPEG(oFile) }; function loadAllImages() {
                var aImages = document.getElementsByTagName("img"); for (var i =
                    0; i < aImages.length; i++)if (aImages[i].getAttribute("exif") == "true") if (!aImages[i].complete) addEvent(aImages[i], "load", function () { EXIF.getData(this) }); else EXIF.getData(aImages[i])
...
var ImageInfo = {};
(function () {
    var files = []; function readFileData(filename, binaryData, binaryLength, callback) { var data = new BinaryFile(binaryData, 0, binaryLength); var tags = readInfoFromData(data); files[filename] = tags; if (callback) callback() } function readInfoFromData(data) {
        var offset = 0; if (data.getByteAt(0) == 255 && data.getByteAt(1) == 216) return readJPEGInfo(data); if (data.getByteAt(0) == 137 && data.getStringAt(1, 3) == "PNG") return readPNGInfo(data); if (data.getStringAt(0, 3) == "GIF") return readGIFInfo(data); if (data.getByteAt(0) == 66 &&
            data.getByteAt(1) == 77) return readBMPInfo(data); return { format: "UNKNOWN" }
    } function readPNGInfo(data) { var w = data.getLongAt(16, true); var h = data.getLongAt(20, true); var bpc = data.getByteAt(24); var ct = data.getByteAt(25); var bpp = bpc; if (ct == 4) bpp *= 2; if (ct == 2) bpp *= 3; if (ct == 6) bpp *= 4; var alpha = data.getByteAt(25) >= 4; return { format: "PNG", version: "", width: w, height: h, bpp: bpp, alpha: alpha, exif: {} } } function readGIFInfo(data) {
        var version = data.getStringAt(3, 3); var w = data.getShortAt(6); var h = data.getShortAt(8); var bpp = (data.getByteAt(10) >>
            4 & 7) + 1; return { format: "GIF", version: version, width: w, height: h, bpp: bpp, alpha: false, exif: {} }
    } function readJPEGInfo(data) {
        var w = 0; var h = 0; var comps = 0; var len = data.getLength(); var offset = 2; while (offset < len) { var marker = data.getShortAt(offset, true); offset += 2; if (marker == 65472) { h = data.getShortAt(offset + 3, true); w = data.getShortAt(offset + 5, true); comps = data.getByteAt(offset + 7, true); break } else offset += data.getShortAt(offset, true) } var exif = {}; if (typeof EXIF != "undefined" && EXIF.readFromBinaryFile) exif = EXIF.readFromBinaryFile(data);
        return { format: "JPEG", version: "", width: w, height: h, bpp: comps * 8, alpha: false, exif: exif }
    } function readBMPInfo(data) { var w = data.getLongAt(18); var h = data.getLongAt(22); var bpp = data.getShortAt(28); return { format: "BMP", version: "", width: w, height: h, bpp: bpp, alpha: false, exif: {} } } ImageInfo.loadInfo = function (args, cb) { if (!files[args.filename]) readFileData(args.filename, args.binary, args.size, cb); else if (cb) cb() }; ImageInfo.getAllFields = function (filename) {
        if (!files[filename]) return null; var tags = {}; for (var a in files[filename]) if (files[filename].hasOwnProperty(a)) tags[a] =
            files[filename][a]; return tags
    }; ImageInfo.getField = function (filename, field) { if (!files[filename]) return null; return files[filename][field] }
})();